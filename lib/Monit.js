// idup-htop
// Library who interacts with IDUP to display processes resources in htop way
...
Monit.init = function(processes, debug) {
  ...
  Monit.multi.write('\x1B[32m⌬ IDUP \x1B[39mmonitoring :\n\n');

  processes.forEach(function(proc, i) {
    if (proc.status == 'stopped' || proc.status == 'stopped - too fast exit')
      return ;

    var process_name = proc.idup_env.name || p.basename(proc.idup_env.pm_exec_path);
    var status = proc.idup_env.status == 'online' ? '●'.green.bold : '●'.red.bold;

    Monit.multi.write(' ' + status + ' ' + process_name.green.bold);
    Monit.multi.write('\n');
    Monit.multi.write('[' + proc.idup_env.pm_id + '] [' + proc.idup_env.exec_mode + ']\n');
