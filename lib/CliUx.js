...
UX.miniDisplay = function(list) {
  list.forEach(function(l) {
    ...
    var mode = l.idup_env.exec_mode.split('_mode')[0];
    var status = l.idup_env.status;
    var port = l.idup_env.port;
    var key = l.idup_env.name || p.basename(l.idup_env.pm_exec_path.script);

    console.log('+--- %s', key);
    console.log('pid : %s', l.pid);
    console.log('idup id : %s', l.idup_env.pm_id);
    console.log('status : %s', status);
    console.log('restarted : %d', l.idup_env.restart_time ? l.idup_env.restart_time : 0);
    console.log('uptime : %s', (l.idup_env.pm_uptime && status == 'online') ? timeSince(l.idup_env.pm_uptime) : 0);
    console.log('memory usage : %s', l.monit ? UX.bytesToSize(l.monit.memory, 3) : '');
    console.log('out log : %s', l.idup_env.pm_out_log_path);
    console.log('error log : %s', l.idup_env.pm_err_log_path);
    console.log('PID file : %s\n', [l.idup_env.pm_pid_path, l.pm_id, '.pid'].join(''));
  });
...
UX.dispAsTable = function(list) {
  ...
  list.forEach(function(l) {
    ...
    var status = l.idup_env.status;
    var port = l.idup_env.port;
    var key = l.idup_env.name.bold || p.basename(l.idup_env.pm_exec_path.script).bold;
    ...
    obj[key] = [
      ...
      port ? port : '',
      l.idup_env.restart_time ? l.idup_env.restart_time : 0,
      (l.idup_env.pm_uptime && status == 'online') ? timeSince(l.idup_env.pm_uptime) : 0,
      l.monit ? UX.bytesToSize(l.monit.memory, 3) : '',
      l.idup_env.pm_err_log_path
    ];
