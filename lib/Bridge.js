...
/**
 *
 * Start the daemon if not yet started
 * @api public
 */
Bridge.start = function(cb) {
  Bridge.pingDaemon(function(alive) {
    if (!alive) {
      setTimeout(function() {
      return Bridge.launchDaemon(function(err, child) {
        if (err) {
        ...
        console.log(cst.PREFIX_MSG + "Started");
        return Bridge.launchRPC(cb);
      });
      }, 1000);
    }
    return Bridge.launchRPC(cb);
